...
interface Props {
  ...
  description?: string;
  landing: string;
  role: string; // Nuevo prop para definir el rol
}
    ...
    <div className="relative w-[205px] h-[290px] mx-auto flip-card cursor-pointer">
      <div className="flip-card-inner w-full h-full relative">
        ...
        <div className="flip-card-back flex flex-col gap-4 absolute w-full h-full bg-gray-900 rounded-2xl px-4 py-4">
          ...
          <span className="bg-gradient-to-r from-[#fe44fd] via-[#eabbbe] to-[#8731ef] inline-block text-transparent font-semibold bg-clip-text text-right absolute bottom-4 right-4"></span>
        </div>
      </div>
    </div>
  );
