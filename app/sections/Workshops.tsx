...
let THEME_WORKSHOPS: Record<string, { selected: { background: string; border: string }; unselected: string; noCode: { background: string; border: string; unselected: string } }> = {
	// '': '#a2fbf7',
	'barranqui-ia': {
		'selected': {
			background: '#d3a2fb',
...
const WORKSHOPS: Record<string, { title: string; name: string; role: string, selected: boolean, confirmed?: boolean, description?: string, image?: string, type?: string, time?: string }[]> = {
	'barranqui-ia': [
		{
...
export default function Workshops({ landing }: Props) {
	...
	const renderCompactWorkshopsList = (workshopsList, isNoCode = false) => {
		// Filtrar solo los talleres confirmados
		const confirmedWorkshops = workshopsList.filter(workshop => workshop.confirmed);
