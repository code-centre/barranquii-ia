...
export default function ProponUnRetoPage() {
  ...
  return (
    <main className="min-h-screen flex flex-col">
      ...
      {/* ── 2. PAIN POINTS: "¿Te suena familiar?" ── */}
      <section
      ...
      >
        <div className="max-w-6xl mx-auto px-5 lg:px-10">
          <div className="text-xs uppercase tracking-widest text-[#FF97EF] font-medium mb-3">
            {t('proponReto.painTag')}
          </div>
          <h2 className="text-2xl md:text-3xl lg:text-4xl font-bold text-white mb-4">
            {t('proponReto.painTitle')}
          </h2>
          <p className="text-white/70 max-w-xl mb-12">
            {t('proponReto.painSub')}
          </p>
          ...
          <div className="grid md:grid-cols-3 gap-4">
            {[1, 2, 3].map((i) => (
              <article
                key={i}
                className="venture-card p-6 md:p-8 rounded-2xl hover:border-[#FF97EF]/40 transition-colors"
              >
                <div className="text-3xl mb-4">{t(`proponReto.pain${i}Icon`)}</div>
                <h3 className="text-lg font-bold text-white mb-3 leading-snug">
                  {t(`proponReto.pain${i}Title`)}
                </h3>
                <p className="text-sm text-white/60 leading-relaxed">
                  {t(`proponReto.pain${i}Desc`)}
                </p>
              </article>
            ))}
          </div>
      ...
      >
        <div className="max-w-6xl mx-auto px-5 lg:px-10">
          <div className="text-xs uppercase tracking-widest text-[#FF97EF] font-medium mb-3">
            {t('proponReto.shiftTag')}
          </div>
          <h2 className="text-2xl md:text-3xl lg:text-4xl font-bold text-white mb-6 leading-tight max-w-3xl">
          ...
          <button
            type="button"
            onClick={scrollToStages}
            className="text-[#FF97EF] hover:text-white transition-colors font-medium"
          >
            {t('proponReto.shiftCta')}
          </button>
        </div>
      </section>

      {/* ── 3b. SOCIAL PROOF: Real examples ── */}
      <section
        className="py-20 md:py-28"
        style={{ backgroundColor: '#1C1F2E' }}
      >
        <div className="max-w-6xl mx-auto px-5 lg:px-10">
          <div className="text-xs uppercase tracking-widest text-[#FF97EF] font-medium mb-3">
            {t('proponReto.proofTag')}
          </div>
          <h2 className="text-2xl md:text-3xl lg:text-4xl font-bold text-white mb-4">
            {t('proponReto.proofTitle')}
          </h2>
          <p className="text-white/70 max-w-xl mb-12">
            {t('proponReto.proofSub')}
          </p>
          ...
          <div className="grid md:grid-cols-2 gap-4">
            {[1, 2, 3, 4].map((i) => (
              <article
                key={i}
                className="venture-card p-6 rounded-xl flex flex-col"
              >
                <div className="flex items-baseline justify-between gap-3 mb-3">
                  <h3 className="font-bold text-white">{t(`proponReto.proof${i}Company`)}</h3>
                </div>
                <p className="text-[#FF97EF] font-semibold text-sm mb-2">
                  {t(`proponReto.proof${i}Result`)}
                </p>
                <p className="text-sm text-white/60 leading-relaxed">
                  {t(`proponReto.proof${i}Desc`)}
                </p>
              </article>
            ))}
          </div>
        ...
        <div className="max-w-6xl mx-auto px-5 lg:px-10">
          ...
          <div className="grid md:grid-cols-2 gap-4">
            ...
            <article className="venture-card p-6 md:p-8 rounded-2xl relative">
              ...
              <p className="text-sm text-white/60 mb-6 leading-relaxed">{t('proponReto.pkgExploradorDesc')}</p>

              {(['1', '2', '3'] as const).map((cat) => (
                <div key={cat} className="mb-4 last:mb-0">
                  <div className="text-xs uppercase tracking-wider text-[#FF97EF]/70 font-medium mb-2">
                    {t(`proponReto.pkgExpCat${cat}`)}
                  </div>
                  <ul className="space-y-1.5 text-sm text-white/90">
                    <li className="flex gap-2">
                      <span className="text-[#00796B] font-bold shrink-0">✓</span>
                      {t(`proponReto.pkgExpCat${cat}a`)}
                    </li>
                    <li className="flex gap-2">
                      <span className="text-[#00796B] font-bold shrink-0">✓</span>
                      {t(`proponReto.pkgExpCat${cat}b`)}
                    </li>
                  </ul>
                </div>
              ))}
            ...
            <article className="venture-card p-6 md:p-8 rounded-2xl relative border-2 border-[#FF97EF]">
              ...
              <p className="text-sm text-white/60 mb-6 leading-relaxed">{t('proponReto.pkgTransformacionDesc')}</p>

              <div className="text-sm text-white/90 font-medium mb-4 pb-4 border-b border-white/10">
                {t('proponReto.pkgTraIncludes')}
              </div>

              {(['1', '2'] as const).map((cat) => (
                <div key={cat} className="mb-4 last:mb-0">
                  <div className="text-xs uppercase tracking-wider text-[#FF97EF]/70 font-medium mb-2">
                    {t(`proponReto.pkgTraCat${cat}`)}
                  </div>
                  <ul className="space-y-1.5 text-sm text-white/90">
                    {['a', 'b', 'c'].map((letter) => (
                      <li key={letter} className="flex gap-2">
                        <span className="text-[#00796B] font-bold shrink-0">✓</span>
                        {t(`proponReto.pkgTraCat${cat}${letter}`)}
                      </li>
                    ))}
                  </ul>
                </div>
      ...
      >
        <div className="max-w-6xl mx-auto px-5 lg:px-10">
          <div className="p-6 md:p-8 rounded-xl bg-white/5 border border-white/10 flex flex-col md:flex-row md:items-center gap-6">
